...
-- restore <spriteName> <x>,<y>,<z>
-- Restores a removed tile object at given coordinates
SafeZoneCommands.register("restore", function(args)
    if #args < 2 then
        log("restore: usage — restore <spriteName> <x>,<y>,<z>")
        return
    end
    local spriteName = args[1]
    local coords = args[2]
    local cx, cy, cz = coords:match("([^,]+),([^,]+),([^,]+)")
    local x = tonumber(cx)
    local y = tonumber(cy)
    local z = tonumber(cz)
    if not x or not y or not z then
        log("restore: bad coords — " .. coords)
        return
    end
    local square = getSquare(x, y, z)
    if not square then
        log("restore: square not loaded at " .. coords)
        return
    end
    createTile(spriteName, square)
    local objects = square:getObjects()
    if objects:size() > 0 then
        square:transmitAddObjectToSquare(objects:get(objects:size() - 1), -1)
    end
    log("restore: " .. spriteName .. " at " .. coords)
end)

-- msguser <username> <text...>
-- Sends personal message to specific player
SafeZoneCommands.register("msguser", function(args)
    ...
    for i = 0, players:size() - 1 do
        local p = players:get(i)
        if p:getUsername():lower() == username:lower() then
            sendServerCommand(p, "SZ_Sync", "notify", {text = text})
