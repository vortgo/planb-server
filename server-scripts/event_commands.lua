...
local function onClientCommand(module, command, player, args)
    ...
    elseif command == "reload" then
        local ok = SafeZoneConfig.loadExternal()
        local msg
        if ok then
            msg = "Config reloaded from Zomboid/Lua/SafeZone_config.lua"
        else
            msg = "Config reload failed (check server log)"
        end
        sendServerCommand(player, "SafeZoneEvents", "result", { text = msg })
    ...
    SafeZoneCommands.register("event", function(args)
        ...
        elseif sub == "reload" then
            ...
            if ok then
                ...
                log("Config reload failed (check log above)")
            end
