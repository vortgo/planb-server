...
function ISBoardCreate:createChildren()
    ...
    self.boardForOnlyAdmins = ISTickBox:new(x, self.nameBoard:getBottom() + 10, self.width-x*2, 30, "", nil, nil)
    self.boardForOnlyAdmins.tooltip = getText("UI_SC_OnlyAdminsBoard");
    self.boardForOnlyAdmins:initialise()
    self.boardForOnlyAdmins:instantiate()
    self.boardForOnlyAdmins:addOption(getText("UI_SC_OnlyAdmins"));
    self:addChild(self.boardForOnlyAdmins)
    
    print("[SC_Board] ISBoardCreate: SC_isAdmin()=" .. tostring(SC_isAdmin and SC_isAdmin() or "UNDEFINED"))
    if SC_isAdmin and not SC_isAdmin() then
